...
Public Class Wd_Societe
    Public imagebyte As Byte() = Nothing

    Private Sub Wd_Societe_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles Me.Closing
        If CTreso_Connect.gStateSociete = True Then
            If MyConn.AddSociete(Trim(TbRaisonSoc.Text), Trim(TbProfession.Text), Trim(TbAdresse.Text), Trim(TbComplement.Text), Trim(TbCP.Text), Trim(TbVille.Text), Trim(TbPays.Text), Trim(TbCommentaire.Text), _
                                 Trim(TbIdentifiant.Text), FormDate(Trim(DP_DebutExo.Text)), "", "", "", "", FormDate(Trim(DP_finExo.Text)), "", "", "", "", Trim(TbTelephone.Text), Trim(TbTeleponie.Text), _
                                 Trim(TbEmail.Text), Trim(TbSite.Text), Trim(TbExercice.Text), imagebyte) Then
                ...
                End If
            Else
                MessageBox.Show("Error lors de l'enregistrement des infos de votre Société ")
            End If
        Else
    ...
    Private Sub loadSociete()
        ...
        imagebyte = d_socete.D_Imagebyte
        Dim lstream As New IO.MemoryStream(pictureData)
        'Dim limage As Image = Image.FromStream(lstream)

        Dim bmImage As BitmapImage = New BitmapImage()
        bmImage.BeginInit()
        bmImage.StreamSource = lstream
        bmImage.EndInit()

        Img_logo.Source = bmImage
    ...
    Private Sub Btn_logo_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs) Handles Btn_logo.Click
        ...
        If result = True Then
            ...
            imagebyte = System.IO.File.ReadAllBytes(filename)
            '
            Dim bmImage As BitmapImage = New BitmapImage()
            bmImage.BeginInit()
